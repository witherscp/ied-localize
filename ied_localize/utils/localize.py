...
def lead_gm_velocities(
    ...
    step_indices = np.where(overlapping)[0]
    source_nodes = np.where(overlapping)[1]
    shuffle_indices = np.where(overlapping)[2]
...
def lead_wm_velocities(
    ...
    master_min_vs, master_max_vs = np.array(()), np.array(())
    ...
    for i in range(len(l_parc_idxs)):
        ...
        shuffle_indices = np.where(overlapping)[3]
    ...
    if master_min_vs.size == 0:
        ...
        return (np.average(master_min_vs), np.average(master_max_vs))
...
if __name__ == "__main__":
  ...
  parser.add_argument("--only_geo", action='store_true',
                      help='use geodesic localization only')
  parser.add_argument("--only_wm", action='store_true',
                      help='use white matter localization only')
  parser.add_argument("-p", "--num_parc", default=600,
                      help=("number of parcels to use to generate FC matrix;"
                      " defaults to 600"))
  parser.add_argument("-n", "--num_network", default=17,
                      help="Yeo network {7 or 17}; defaults to 17")
  parser.add_argument("--dist", default=45,
                      help="Geodesic max search distance; defaults to 45")
  parser.add_argument("-l","--max_length", default=10,
                      help='maximum allowable length of a sequence')

  args = parser.parse_args()
  subj = args.subj
  n_parcs = int(args.num_parc); n_network = int(args.num_network)
  dist = int(args.dist); max_length = int(args.max_length)
  only_geo = args.only_geo; only_wm = args.only_wm
  ...
  for cluster in range(1,s.num_clusters+1):
      ...
      opath = odir / (f"{cluster_hemi}{file_str}_sourceParcels_within{s.dist}"
      ...
      print(Colors.GREEN, 
            f"Source parcel file created at {opath}", 
            Colors.END)

      # output normalized counts
      opath = odir / (f"{cluster_hemi}{file_str}_normalizedCounts_within"
                      f"{s.dist}_max{s.seq_len}_cluster{cluster}.csv")
      sources_dict = convert_list_to_dict(all_source_parcs)
      output_df = output_normalized_counts(seqs.shape[0], 
                                            sources_dict,
                                            s.parcs)
      output_df.to_csv(opath)
      print(Colors.GREEN, 
            f"Normalized counts file created at {opath}",
            Colors.END)